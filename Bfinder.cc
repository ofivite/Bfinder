...
Bfinder::Bfinder(const edm::ParameterSet& iConfig)
  ...
  tree_(0), 
   ...
   B_mass(0),         B_mass_c0(0),     B_mass_0c(0),
   ...
   B_pion_px1(0),       B_pion_py1(0),  B_pion_pz1(0),
   B_pion_charge1(0),   pion1_trackchi2(0),     pion1_Hits(0),  pion1_PHits(0),
   pion1_dxy_Bcdecay(0), pion1_dz_Bcdecay(0), pion1_NTrackerLayers(0),  pion1_NPixelLayers(0), pi1_Bcdecay_weight(0), pi1_CommonVtx_weight(0),
   ...
   pion2_dxy_PV(0), pion2_dz_PV(0),   pion2_dxy_Bcdecay(0), pion2_dz_Bcdecay(0), pion2_NTrackerLayers(0),  pion2_NPixelLayers(0), pi2_PV_weight(0), pi2_CommonVtx_weight(0),
   ...
   BcVertex_isValid(0), BcVertex_Chi(0), BcVertex_normChi(0), CommonVtx_isValid(0), CommonVtx_Chi(0), CommonVtx_normChi(0), 
   comp_BcVtx_CommonVtx(0), dist_BcVtx_CommonVtx(0), DS_BcVtx_CommonVtx(0),
   run(0), event(0)
...
//		if (patTrack1.track()->hitPattern().pixelLayersWithMeasurement() <= 1) continue; 
               ...
               TLorentzVector p4pi1, p4bc;
               p4pi1.SetXYZM(iTrack1->px(),iTrack1->py(),iTrack1->pz(), PDG_KAON_MASS);

               p4bc = p4mu1_0c + p4mu2_0c + p4pi1;
               if(p4bc.M() > 6.6) continue;
               if(p4bc.M() < 4.) continue;
               ...
               B_candidate_init.push_back(pFactory.particle(pion1TT, PM_PDG_KAON_MASS, chi,ndf, PM_muon_sigma));
    ...
    ///////////////////////////////  BS VERTEX FIT /////////////////////////////////////////
               ...
               if(B_mass_cjp_tmp < 5.1) continue;
               if(B_mass_cjp_tmp > 5.45) continue;
               //
     ...
     /////////////////////////////////// // try refitting the primary without the tracks in the B reco candidate
              ...
              {     
                    ...
                    // the 4 tracks in the B cand are  patTrack1 trkTrackP trkTrackM
...
//		if (patTrack2.track()->hitPattern().pixelLayersWithMeasurement() <= 1) continue; 
                   ...
                   for ( std::vector<TrackBaseRef >::const_iterator iTrack_a = bestVtxRf.tracks_begin(); iTrack_a != bestVtxRf.tracks_end(); ++iTrack_a) 
                   {
                       ...
                       if (  (patTrack2.track()==trackRefpion) ) 
                       {
                   ...
                   p4pi2.SetXYZM(iTrack2->px(),iTrack2->py(),iTrack2->pz(), PDG_KAON_MASS);

                   if ((p4bc + p4pi2).M()  - B_mass_cjp_tmp - PDG_KAON_MASS > 0.1)  continue;
...
///~~~fit all 5 tracks together~~~///
	vector<reco::TransientTrack> FiveTracks;
        ...
        TransientTrack pion2TT(patTrack2.track(), &(*bFieldHandle) );

	FiveTracks.push_back(muon1TT);
	FiveTracks.push_back(muon2TT);
	FiveTracks.push_back(pion1TT);
	FiveTracks.push_back(pion2TT);
	
        AdaptiveVertexFitter FiveTrackFitter;
        TransientVertex _TV = FiveTrackFitter.vertex(FiveTracks, BcGP);

	CommonVtx_isValid->push_back(_TV.isValid());	 
        if ( _TV.isValid() ) 
	{
           CommonVtx = reco::Vertex(_TV);  
	   pi1_CommonVtx_weight->push_back(CommonVtx.trackWeight(patTrack1.track()));
	   pi2_CommonVtx_weight->push_back(CommonVtx.trackWeight(patTrack2.track()));
	...
	{
	   pi1_CommonVtx_weight->push_back(-1);
	   pi2_CommonVtx_weight->push_back(-1);
...
///~~~Distance and Compatibility~~~///
	...
	comp_BcVtx_CommonVtx->push_back( vertTool.compatibility(BcVtx, CommonVtx) );
	dist_BcVtx_CommonVtx->push_back( vertTool.distance(BcVtx, CommonVtx).value() );
	DS_BcVtx_CommonVtx->push_back( vertTool.distance(BcVtx, CommonVtx).significance() );
        ...
        //                B_DecayVtxYZE->push_back(bDecayVertexCjp->error().czy());
                       ...
                       B_pion_px1       ->push_back(T1CandMC->currentState().globalMomentum().x());
                       B_pion_py1       ->push_back(T1CandMC->currentState().globalMomentum().y());
                       B_pion_pz1       ->push_back(T1CandMC->currentState().globalMomentum().z());
                       B_pion_charge1   ->push_back(iTrack1->charge());
                       pion1_trackchi2  ->push_back(patTrack1.track()->normalizedChi2());
                       pion1_Hits       ->push_back(patTrack1.track()->numberOfValidHits() );
                       pion1_PHits      ->push_back(patTrack1.track()->hitPattern().numberOfValidPixelHits() );	
                       pion1_dxy_Bcdecay	->push_back(patTrack1.track()->dxy(bDecayPoint) );
                       pion1_dz_Bcdecay		->push_back(patTrack1.track()->dz(bDecayPoint	) );
		       pion1_NTrackerLayers->push_back ( patTrack1.track()->hitPattern().trackerLayersWithMeasurement() );
		       pion1_NPixelLayers->push_back ( patTrack1.track()->hitPattern().pixelLayersWithMeasurement() ); 
        ...
        //                B_J_charge2->push_back(mu2CandMC->currentState().particleCharge());
                       //
               //
                       B_pion_px2       ->push_back(p4pi2.Px());
                       B_pion_py2       ->push_back(p4pi2.Py());
                       B_pion_pz2       ->push_back(p4pi2.Pz());
                       B_pion_charge2   ->push_back(iTrack2->charge());
                       pion2_trackchi2  ->push_back(patTrack2.track()->normalizedChi2());
                       pion2_Hits       ->push_back(patTrack2.track()->numberOfValidHits() );
                       pion2_PHits      ->push_back(patTrack2.track()->hitPattern().numberOfValidPixelHits() );	

                       pion2_dxy_PV	->push_back(patTrack2.track()->dxy(bestVtxRf.position()) );
                       pion2_dxy_Bcdecay	->push_back(patTrack2.track()->dxy(bDecayPoint) );
                       pion2_dz_PV		->push_back(patTrack2.track()->dz(bestVtxRf.position()) );
                       pion2_dz_Bcdecay		->push_back(patTrack2.track()->dz(bDecayPoint) );

		       pion2_NTrackerLayers->push_back ( patTrack2.track()->hitPattern().trackerLayersWithMeasurement() );
		       pion2_NPixelLayers->push_back ( patTrack2.track()->hitPattern().pixelLayersWithMeasurement() ); 
		       pi2_PV_weight ->push_back ( bestVtxRf.trackWeight(patTrack2.track()) ); 
...
//                       } // track 3
   ...
   B_mass->clear();         B_mass_c0->clear();     B_mass_0c->clear();
   ...
   B_pion_px1->clear();     B_pion_py1->clear();    B_pion_pz1->clear();
   B_pion_charge1->clear(); pion1_trackchi2->clear();   pion1_Hits->clear();    pion1_PHits->clear();
   pion1_dxy_Bcdecay->clear();  pion1_dz_Bcdecay->clear(); pion1_NTrackerLayers->clear();  pion1_NPixelLayers->clear();
   pi1_Bcdecay_weight->clear(); pi1_CommonVtx_weight->clear();
   ...
   pion2_dxy_PV->clear();  pion2_dxy_Bcdecay->clear(); pion2_dz_PV->clear(); pion2_dz_Bcdecay->clear(); pion2_NTrackerLayers->clear();  pion2_NPixelLayers->clear(); pi2_PV_weight->clear();
   pi2_CommonVtx_weight->clear();
   //
   ...
   BcVertex_isValid->clear(); BcVertex_Chi->clear(); BcVertex_normChi->clear(); CommonVtx_isValid->clear(); CommonVtx_Chi->clear(); CommonVtx_normChi->clear(); 
   comp_BcVtx_CommonVtx->clear(); dist_BcVtx_CommonVtx->clear(); DS_BcVtx_CommonVtx->clear();
   // }}}
}
...
void Bfinder::beginJob()
{
  ...
  tree_->Branch("B_pion_px1"        , &B_pion_px1           );
  tree_->Branch("B_pion_py1"        , &B_pion_py1           );
  tree_->Branch("B_pion_pz1"        , &B_pion_pz1           );
  tree_->Branch("B_pion_charge1"    , &B_pion_charge1       );
  tree_->Branch("pion1_trackchi2"   , &pion1_trackchi2      );
  tree_->Branch("pion1_Hits"        , &pion1_Hits           );
  tree_->Branch("pion1_PHits"       , &pion1_PHits          );
  tree_->Branch("pion1_dxy_Bcdecay"         , &pion1_dxy_Bcdecay          );
  tree_->Branch("pion1_dz_Bcdecay"          , &pion1_dz_Bcdecay          );
  tree_->Branch("pion1_NTrackerLayers"       , &pion1_NTrackerLayers          );
  tree_->Branch("pion1_NPixelLayers"       , &pion1_NPixelLayers          );
  tree_->Branch("pi1_Bcdecay_weight"       , &pi1_Bcdecay_weight          );
  tree_->Branch("pi1_CommonVtx_weight"       , &pi1_CommonVtx_weight          );
  ...
  tree_->Branch("pion2_dxy_Bcdecay"         , &pion2_dxy_Bcdecay          );
  tree_->Branch("pion2_dz_PV"          , &pion2_dz_PV          );
  tree_->Branch("pion2_dz_Bcdecay"         , &pion2_dz_Bcdecay          );
  ...
  tree_->Branch("pi2_CommonVtx_weight"       , &pi2_CommonVtx_weight          );
...
//////------------////////
  ...
  tree_->Branch("comp_BcVtx_CommonVtx"        , &comp_BcVtx_CommonVtx           );
  tree_->Branch("dist_BcVtx_CommonVtx"        , &dist_BcVtx_CommonVtx           );
  tree_->Branch("DS_BcVtx_CommonVtx"        , &DS_BcVtx_CommonVtx           );
